...
def draw_boxes(image, boxes, line, labels, obj_thresh, dcnt):
    ...
    for box in boxes:
        ...
        if label >= 0:
            ...
            #print(tf)
            if tf:
                cx,cy=get_centroid(box.xmin, box.ymin,box.xmax, box.ymax)
                lineHeight=(line[0][1] + line[1][1])/2
                if(cy<lineHeight + offset and cy>lineHeight - offset):
                    vehicle_count[labels[label]]+=1

            cv2.line(image, line[0], line[1], (255, 0, 0), 3)
            centro=get_centroid(box.xmin, box.ymin,box.xmax, box.ymax)
            if tf:
                cv2.rectangle(image, (box.xmin,box.ymin), (box.xmax,box.ymax), (255,0,0), 3)
                cv2.circle(image, centro, 4, (255, 0,0), -1)
                print('Box: (' + str(box.xmin) + ', ' + str(box.ymin) + ') (' + str(box.xmax) + ', ' + str(box.ymax) + ')')
                #cimg = image[box.ymin:box.ymax, box.xmin:box.xmax]
                #cv2.imshow("violation", cimg)
                #cv2.waitKey(5)
                #cv2.imwrite(r"C:\Users\Asus\Desktop\sakib\Detected Images\violation_"+str(dcnt)+".jpg", cimg)
                #dcnt = dcnt+1
            else:
                cv2.rectangle(image, (box.xmin,box.ymin), (box.xmax,box.ymax), (0,255,0), 3)
                cv2.circle(image, centro, 4, (0, 255,0), -1)
...
def refresh_vehicle_count():
    vehicle_count = {"person":0, "bicycle":0, "car":0, "motorbike":0, "aeroplane":0, "bus":0, "train":0, "truck":0}


weights_path = r"C:\Users\Asus\Desktop\sakib\yolov3.weights"
# set some parameters
net_h, net_w = 416, 416
